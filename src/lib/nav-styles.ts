...
/** Theme-aware header — light surface in light mode, sage dark band in dark mode. */
export const headerBarClass =
  "site-header shrink-0 border-b border-border bg-surface/95 text-foreground backdrop-blur-md dark:border-border dark:bg-[color:var(--hero-bg)]/95";
...
export function mobileNavLinkClass(active: boolean) {
  return cn(
    ...
    active
      ? "bg-[color:var(--brand-soft)] text-[color:var(--accent)] dark:bg-surface-muted dark:text-accent"
...
export const headerCtaClass =
  "interactive-scale rounded-lg bg-accent px-4 py-2 text-sm font-semibold text-white shadow-sm transition-transform duration-200 hover:scale-105 hover:bg-[color:var(--brand-light)] active:scale-95 dark:text-[#1f2622]";
